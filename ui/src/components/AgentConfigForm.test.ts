...
describe("resolveCheapProfileBase", () => {
  ...
  it("resets the cheap model when the adapter type was switched (no bleed)", () => {
    expect(resolveCheapProfileBase(runtimeConfig, true)).toEqual({ enabled: true, model: "", adapterConfig: {} });
  });

  it("handles a missing cheap profile", () => {
    expect(resolveCheapProfileBase({}, false)).toEqual({ enabled: true, model: "", adapterConfig: {} });
    expect(resolveCheapProfileBase(null, false)).toEqual({ enabled: true, model: "", adapterConfig: {} });
  });
});
